...
def main():
    ...
    try:
        ...
        # Data cleansing: filter rows where itemcode or last_update_date are null
        ...
        if df.count() > 1:
            ...
            # Drop duplicate column from enriched df
            df = df.drop("reshet_num") 

            # Write Parquet file to the specified bucket
            spark_write_data_to_bucket(df, 'ml-bucket')
        else:
            logger.info("DataFrame has 1 or fewer rows. Skipping write to MinIO.")
